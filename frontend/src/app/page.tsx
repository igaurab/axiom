...
export default function RunsPage() {
  ...
  const [deleteModal, setDeleteModal] = useState<{ ids: number[]; label?: string } | null>(null);
  ...
  const datasetNames = useMemo(() => [...new Set(runs.map((r) => r.suite_name).filter(Boolean))].sort(), [runs]);
  const agentNames = useMemo(() => [...new Set(runs.map((r) => r.agent_name).filter(Boolean))].sort(), [runs]);
  ...
  const toggleAll = useCallback(() => {
    setSelected((prev) => prev.size === filteredRuns.length ? new Set() : new Set(filteredRuns.map((r) => r.id)));
  }, [filteredRuns]);
  ...
  const handleCompare = async () => {
    ...
    setComparing(true);
    try {
      const comparison = await comparisonsApi.create({ run_ids: [...selected] });
      router.push(`/compare/${comparison.id}`);
    } catch (err: unknown) {
      setCompareError(err instanceof Error ? err.message : "Failed to create comparison");
    } finally {
      setComparing(false);
    }
  };
    ...
    <>
      <PageHeader title="Run History">
        <button
          ...
          onClick={() => { setImportResult(""); setImportModal(true); }}
        >
          <Upload size={15} />
          Import JSON
        </button>
      ...
      {isLoading ? (
        <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="flex items-center gap-4 py-4 px-4 border-b border-border last:border-b-0">
              <div className="skeleton w-4 h-4 rounded" />
              <div className="skeleton h-4 w-44" />
              <div className="flex flex-col gap-1"><div className="skeleton h-3 w-28" /><div className="skeleton h-3 w-20" /></div>
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm py-20 text-center">
          ...
          <p className="text-muted text-sm">No runs yet. Create a new run or import JSON results.</p>
        </div>
      ) : (
        /* Runs table in card */
        <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
            <div className="relative flex-1 max-w-sm">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-light pointer-events-none" />
              <input
                type="text"
            ...
            <div className="relative">
              ...
              <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-light pointer-events-none" />
            </div>
            ...
            {(searchQuery || statusFilter !== "all" || datasetFilter !== "all" || agentFilter !== "all") && (
              ...
              <tr className="bg-[var(--surface-hover)] border-b border-border">
                <th className="w-10 py-2.5 px-4">
                  <Checkbox
                    ...
                    indeterminate={selected.size > 0 && selected.size < filteredRuns.length}
                    onChange={toggleAll}
                  />
                </th>
                <th className="py-2.5 px-3 text-left text-muted text-xs font-medium uppercase tracking-wider">Label</th>
                <th className="py-2.5 px-3 text-left text-muted text-xs font-medium uppercase tracking-wider">Dataset / Agent</th>
                <th className="py-2.5 px-3 text-left text-muted text-xs font-medium uppercase tracking-wider">Status</th>
                <th className="py-2.5 px-3 text-left text-muted text-xs font-medium uppercase tracking-wider">Tags</th>
            ...
            <tbody>
              {filteredRuns.map((run) => {
                const elapsed = run.completed_at && run.started_at
                  ...
                  : "";
                const pct = run.progress_total ? ((run.progress_current / run.progress_total) * 100).toFixed(0) : "0";
                return (
                  <tr
                    key={run.id}
                    className={cn(
                  ...
                  >
                    ...
                    <td className="py-3 px-3 font-medium text-foreground capitalize">{run.label}</td>
                    <td className="py-3 px-3">
                      <div className="text-sm text-foreground leading-tight">{run.suite_name || <span className="text-muted-light">&mdash;</span>}</div>
                      <div className="text-xs text-muted-light leading-tight mt-0.5">{run.agent_name || ""}</div>
                    </td>
                    <td className="py-3 px-3">
                      ...
                      {run.status === "running" && (
                        <div className="flex items-center gap-2 mt-1 animate-progress-pulse">
                          ...
                          <span className="text-[11px] text-muted-light">{run.progress_current}/{run.progress_total}</span>
                        </div>
                      )}
                    </td>
                    <td className="py-3 px-3">
                    ...
                    <td className="py-3 px-3 text-sm text-muted">{formatDate(run.created_at)}</td>
                    <td className="py-3 px-2" onClick={(e) => e.stopPropagation()}>
                      <button
                        ...
                        onClick={() => setDeleteModal({ ids: [run.id], label: run.label })}
                      >
                        <Trash2 size={15} />
                      </button>
                    </td>
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 float-up">
          ...
          <div className="glass rounded-2xl px-5 py-3 flex items-center gap-4 shadow-2xl">
            ...
            <div className="w-px h-5 bg-border" />
            <button
              onClick={() => { setSelected(new Set()); setCompareError(""); }}
      ...
      {/* Import Modal */}
      {importModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center modal-backdrop" onClick={(e) => e.target === e.currentTarget && setImportModal(false)}>
          <div className="bg-card border border-border rounded-2xl w-[90%] max-w-[700px] max-h-[85vh] overflow-y-auto p-6 shadow-2xl modal-content">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-foreground">Import Run from JSON</h3>
              <button className="p-1.5 rounded-lg text-muted-light hover:text-foreground hover:bg-[var(--surface-hover)] transition-colors" onClick={() => setImportModal(false)}>
                <X size={18} />
              </button>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); importMutation.mutate(); }}>
              <div className="mb-4">
                <label className="block font-medium text-sm text-muted mb-1.5">Label</label>
                <input className={inputCls} value={importLabel} onChange={(e) => setImportLabel(e.target.value)} required />
              </div>
              <div className="mb-4">
                <label className="block font-medium text-sm text-muted mb-1.5">JSON Directory</label>
                <div className="flex gap-2">
                  <input className={cn(inputCls, "flex-1")} value={importJsonDir} onChange={(e) => setImportJsonDir(e.target.value)} required placeholder="~/benchmark_app_data/run_0/json" />
                  <button type="button" className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium bg-card border border-border text-foreground hover:bg-[var(--surface-hover)] transition-colors whitespace-nowrap" onClick={openBrowser}>
                    <FolderOpen size={15} />
                    Browse
                  </button>
                </div>
                <span className="text-xs text-muted-light mt-1 block">Path to folder with 1.json, 2.json, ... files</span>
              </div>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block font-medium text-sm text-muted mb-1.5">Dataset</label>
                  <select className={selectCls} value={importSuiteId} onChange={(e) => setImportSuiteId(e.target.value)} required>
                ...
                <div>
                  ...
                  <select className={selectCls} value={importAgentId} onChange={(e) => setImportAgentId(e.target.value)} required>
                    <option value="">Select...</option>
              ...
              <div className="mb-4">
                ...
                <input className={inputCls} value={importTags} onChange={(e) => setImportTags(e.target.value)} />
              </div>
              {importResult && <div className={`text-sm mb-2 ${importResult.startsWith("Imported") ? "text-success" : "text-destructive"}`}>{importResult}</div>}
              <div className="flex justify-end gap-3 mt-6">
                <button type="button" className="px-4 py-2 rounded-xl font-medium text-sm bg-card border border-border text-foreground hover:bg-[var(--surface-hover)] transition-colors" onClick={() => setImportModal(false)}>Cancel</button>
                <button type="submit" className="px-4 py-2 bg-primary text-primary-foreground rounded-xl font-medium text-sm shadow-lg shadow-primary/25 hover:brightness-110 hover:-translate-y-px transition-all">Import</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {browseModal && browseData && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center modal-backdrop" onClick={(e) => e.target === e.currentTarget && setBrowseModal(false)}>
          <div className="bg-card border border-border rounded-2xl w-[90%] max-w-[700px] max-h-[85vh] overflow-y-auto p-6 shadow-2xl modal-content">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-foreground">Select Folder</h3>
              <button className="p-1.5 rounded-lg text-muted-light hover:text-foreground hover:bg-[var(--surface-hover)] transition-colors" onClick={() => setBrowseModal(false)}>
                <X size={18} />
              </button>
            </div>
            <div className="flex gap-2 mb-3">
              ...
              <input className={cn(inputCls, "flex-1 font-mono")} value={browseData.current} readOnly />
            </div>
            <div className="max-h-[350px] overflow-y-auto rounded-xl border border-border bg-[var(--surface)]">
              {browseData.items.length === 0 ? (
                <div className="p-8 text-center text-muted-light italic">Empty folder</div>
                ...
                >
                  {item.type === "dir"
                    ...
                    : <FileText size={16} className="text-muted-light shrink-0" />
                  }
                  {item.name}
                </div>
              ))}
            ...
            <div className="text-sm text-muted mt-2">
              {browseData.items.filter((i) => i.type === "dir").length} folder(s), {browseData.items.filter((i) => i.type === "file").length} .json file(s)
            </div>
            <div className="flex justify-end gap-3 mt-4">
              <button className="px-4 py-2 rounded-xl font-medium text-sm bg-card border border-border text-foreground hover:bg-[var(--surface-hover)] transition-colors" onClick={() => setBrowseModal(false)}>Cancel</button>
              <button className="px-4 py-2 bg-primary text-primary-foreground rounded-xl font-medium text-sm shadow-lg shadow-primary/25 hover:brightness-110 hover:-translate-y-px transition-all" onClick={() => { setImportJsonDir(browseData.current); setBrowseModal(false); }}>Select This Folder</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-card border border-border rounded-2xl w-[420px] p-6 shadow-2xl modal-content">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-lg text-foreground">
                {deleteModal.ids.length === 1 ? "Delete Run" : `Delete ${deleteModal.ids.length} Run(s)`}
              </h3>
            ...
            <p className="mb-4 text-foreground">
              {deleteModal.label
                ...
                : <>Delete <strong>{deleteModal.ids.length} run(s)</strong> and their results?</>
              }
            </p>
            <label className="flex items-center gap-2 p-3 rounded-xl text-sm cursor-pointer bg-destructive/10 border border-destructive/20">
              <input type="checkbox" checked={deleteFiles} onChange={(e) => setDeleteFiles(e.target.checked)} className="accent-[var(--destructive)]" />
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                onClick={() => deleteMutation.mutate({ ids: deleteModal.ids, deleteData: deleteFiles })}
              >
